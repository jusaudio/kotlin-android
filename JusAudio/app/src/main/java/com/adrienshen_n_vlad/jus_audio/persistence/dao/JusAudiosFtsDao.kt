...
interface JusAudiosFtsDao {
    ...
    @Query("SELECT * FROM $AUDIOS_TABLE_NAME JOIN $AUDIOS_FTS_TABLE_NAME ON jus_audios_table.rowid == jus_audios_fts_table.rowid WHERE jus_audios_fts_table.audio_search_tags  MATCH :searchQuery GROUP BY jus_audios_table.rowid ORDER BY $ROW_ID ASC LIMIT :limit OFFSET :offset")
