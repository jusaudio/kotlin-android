package com.curiolabs.jusaudio.content_providers

import android.util.Log
import com.curiolabs.jusaudio.persistence.entities.JusAudios
import com.curiolabs.jusaudio.utility_classes.JusAudioConstants.DEFAULT_LANG_ID
        ...
        val dummySearchList = ArrayList<JusAudios>().also {
            for (i in 1..100) {
                ...
                val tagsBuilder = StringBuilder()
                    .append(
                        generateTags(
                            audioTitleOrAuthors = songTitle
                        )
                    )
                    .append(
                        generateTags(
                            audioTitleOrAuthors = songAuthor
                        )
                    )
